...
%\hspace*{2.4em}   and duplicated task graph, $TC$, $TC'$.\\
...
\STATE Find the edge $e_{max}=(c_i,c_j)$ with the maximum weight in $VCPG^t$, and merge $c_i$ and $c_j$ into one cluster.%, denoted as $c_{i}$.
...
\IF{Setting $e$ as inter-core communication still meets $pc$}
...
Then, the effectiveness of our method in optimizing the design vulnerability is tested, and the $CCR$ is set to 1.0 for all the benchmarks. Three performance constraints are tested in the experiments, which are $0.95SL$, $0.9SL$ and $0.85SL$, and the corresponding design vulnerabilities are presented in the Figs. \ref{subfig:risk_0.95sl}, \ref{subfig:risk_0.9sl}, and \ref{subfig:risk_0.85sl}, respectively.
...
\end{tabular}
\caption{Comparison of design vulnerabilities under different performance constraints. \subref{subfig:risk_0.95sl} Design vulnerability with $pc$=0.95$SL$. \subref{subfig:risk_0.9sl} Design vulnerability with $pc$=0.9$SL$. \subref{subfig:risk_0.85sl} Design vulnerability with $pc$=0.85$SL$.}
...
We greatly appreciate the Editor's and the reviewers' insightful and scrupulous reviews of our paper.
...
The authors address the reviewer's comments from the previous round. The authors may consider adding a table summarizing all mathematical symbols to improve the readability of the paper.
...
Referring to Eq. (3), does the performance constraint in this work literally represent the
deadline for task finish time? If so, why not directly using deadline constraints?
...
This paper heavily relies on pseudocodes to explain the key steps of the methodology, The
authors can include flowcharts to help clarify the algorithmic flow.
...
The authors claim a low complexity of $O(m^2)$, in which m is the number of nodes. How comes
that the scheduling approach takes exceptionally long execution times (regarding the CPU
runtime statistics in Table 2)?
